...
logicamente equivalentes. A próxima definição apresenta o conceito de
equivalência lógica.
...
lógica de duas fórmulas possui o inconveniente de que tabelas verdade
são exponenciais no número de variáveis das fórmulas envolvidas. Desta
maneira, a álgebra booleana é uma alternativa mais eficiente para determinar
equivalência de duas fórmulas.

A álgebra booleana é uma forma de raciocínio algébrico sobre
fórmulas, o que, de maneira simples, permite: 1) mostrar que duas
fórmulas são iguais por meio de uma sequência de igualdades e; 2) se
duas fórmulas são consideradas iguais, você pode utilizar esse fato
para adicionar uma nova igualdade a uma sequência já existente. A
noção de ``sequência de igualdades'' é formalizada em termos da
seguinte propriedade: se $a= b$ e $b = c$ então $a = c$.